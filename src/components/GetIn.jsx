...
const GetIn = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post("https://newcheck.evvisolutions.com/api/createmessages", formData);
    ...
    <div id="getIn" className="bg-gray-300 md:flex md:p-10 p-2 max-md:my-5 justify-between mb-8">
      ...
      <div className="md:w-[40%] p-4 flex flex-col gap-5 md:mb-10">
        ...
        <div className="xl:text-lg sm:text-sm">
          <h3 className="highlight-color text-lg mb-3 primary-color font-medium">Our Office Location</h3>
          <p className="xl:text-lg md:text-sm">EnrichMinds Consulting</p>
          <p className="xl:text-lg md:text-sm">No 2/74-5, 1st Cross, Hosur Main Road</p>
          <p className="xl:text-lg md:text-sm">Madiwala, Bengaluru - 560068, Karnataka.</p>
        </div>
        <div>
      ...
      <form className="space-y-4 md:w-[40%] xl:w-[35%] max-md:px-4 max-md:py-4" onSubmit={handleSubmit}>
        {[
          ...
          { label: "Mobile", name: "number", type: "number", placeholder: "Enter your Number" },
        ].map(({ label, name, type, placeholder }) => (
          <div key={name}>
            <label className="block xl:text-lg text-sm font-medium text-gray-700">{label}</label>
            <input
              type={type}
              name={name}
        ...
        <div>
          ...
          <textarea
            ...
            className={`mt-1 block resize-none w-full h-32 px-3 py-2 border ${
              errors.message ? "border-red-500 placeholder-red-500" : "border-gray-300"
            } rounded-md shadow-sm focus:outline-none focus:ring-yellow-500 focus:border-yellow-500 xl:text-lg sm:text-sm`}
        ...
        <div>
          <button type="submit" className="w-full btn-primary">Submit your request</button>
        </div>
      </form>
